...
describe('saveFavorite', () => {
  it('calls addFavorite with a generated FavoriteItem', async () => {
    mockAddFavorite.mockResolvedValue([{ id: 'mock_fav_id_123', uri: 'file:///new.jpg', score: 85, scoreReason: undefined, date: expect.any(String), gridType: 'thirds', suggestion: 'test suggestion', sceneTag: undefined }]);
    const { result } = renderHook(() => useFavorites());
    await waitFor(() => { expect(result.current.loading).toBe(false); });
    await act(async () => { await result.current.saveFavorite('file:///new.jpg', 'thirds', 'test suggestion', undefined, 85, undefined); });
  ...
  it('updates favorites state sorted by score', async () => {
    mockAddFavorite.mockResolvedValue([
    ...
    mockLoadFavorites.mockResolvedValue([{ id: 'f1', uri: 'file:///a.jpg', score: 40, scoreReason: '', date: '2024-01-01', gridType: 'thirds', suggestion: '', sceneTag: '' }]);
    const { result } = renderHook(() => useFavorites());
    await waitFor(() => { expect(result.current.loading).toBe(false); });
    await act(async () => { await result.current.saveFavorite('file:///new.jpg', 'thirds', '', undefined, 90, ''); });
    expect(result.current.favorites).toHaveLength(2);
    expect(result.current.favorites[0].score).toBe(90);
  });

  it('uses default score of 85 when not provided', async () => {
    mockAddFavorite.mockResolvedValue([{ id: 'mock_fav_id_123', uri: 'file:///new.jpg', score: 85, scoreReason: '', date: '2024-01-01', gridType: 'thirds', suggestion: '', sceneTag: '' }]);
    const { result } = renderHook(() => useFavorites());
    await waitFor(() => { expect(result.current.loading).toBe(false); });
    await act(async () => { await result.current.saveFavorite('file:///new.jpg', 'thirds', '', undefined); });
    const addedItem = mockAddFavorite.mock.calls[0][0];
    expect(addedItem.score).toBe(85);
  });
  ...
  it('returns the created FavoriteItem', async () => {
    ...
    await act(async () => { returnedItem = await result.current.saveFavorite('file:///new.jpg', 'thirds', 'sug'); });
...
describe('deleteFavorite', () => {
  it('calls removeFavorite with the given id', async () => {
    mockRemoveFavorite.mockResolvedValue([]);
    mockLoadFavorites.mockResolvedValue([{ id: 'f1', uri: 'file:///a.jpg', score: 80, scoreReason: '', date: '2024-01-01', gridType: 'thirds', suggestion: '', sceneTag: '' }]);
    const { result } = renderHook(() => useFavorites());
    await waitFor(() => { expect(result.current.loading).toBe(false); });
    await act(async () => { await result.current.deleteFavorite('f1'); });
...
describe('refresh', () => {
  it('reloads favorites from storage', async () => {
    mockLoadFavorites
      ...
      .mockResolvedValueOnce([{ id: 'f1', uri: 'file:///a.jpg', score: 70, scoreReason: '', date: '2024-01-01', gridType: 'thirds', suggestion: '', sceneTag: '' }]);
    const { result } = renderHook(() => useFavorites());
    await waitFor(() => { expect(result.current.loading).toBe(false); });
...
describe('sorting behavior', () => {
  it('keeps favorites sorted by score descending after deletion', async () => {
    mockLoadFavorites.mockResolvedValue([
      { id: 'f1', uri: 'file:///a.jpg', score: 95, scoreReason: '', date: '2024-01-01', gridType: 'thirds', suggestion: '', sceneTag: '' },
      { id: 'f2', uri: 'file:///b.jpg', score: 80, scoreReason: '', date: '2024-01-02', gridType: 'thirds', suggestion: '', sceneTag: '' },
      { id: 'f3', uri: 'file:///c.jpg', score: 60, scoreReason: '', date: '2024-01-03', gridType: 'thirds', suggestion: '', sceneTag: '' },
    ]);
    mockRemoveFavorite.mockResolvedValue([
      { id: 'f2', uri: 'file:///b.jpg', score: 80, scoreReason: '', date: '2024-01-02', gridType: 'thirds', suggestion: '', sceneTag: '' },
      { id: 'f3', uri: 'file:///c.jpg', score: 60, scoreReason: '', date: '2024-01-03', gridType: 'thirds', suggestion: '', sceneTag: '' },
