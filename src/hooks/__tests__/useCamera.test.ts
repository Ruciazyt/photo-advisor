...
describe('initial state', () => {
  ...
  it('has cameraRef as a React ref object', () => {
    const { result } = renderHook(() => useCamera());
    expect(result.current.cameraRef).toHaveProperty('current');
  });

  it('permissionGranted is true when permission granted', () => {
    mockUseCameraPermissions.mockReturnValue([{ granted: true }, jest.fn()]);
    const { result } = renderHook(() => useCamera());
    expect(result.current.permissionGranted).toBe(true);
  });

  it('permissionGranted is false when permission not granted', () => {
    mockUseCameraPermissions.mockReturnValue([{ granted: false }, jest.fn()]);
    const { result } = renderHook(() => useCamera());
    expect(result.current.permissionGranted).toBe(false);
  });

  it('rawSupported is set from supportsRawCapture call', async () => {
    mockSupportsRawCapture.mockResolvedValue(false);
    const { result } = renderHook(() => useCamera());
...
describe('switchCamera', () => {
  it('toggles facing from back to front', async () => {
    const { result } = renderHook(() => useCamera());
  ...
  it('toggles facing from front to back', async () => {
    const { result } = renderHook(() => useCamera({ initialMode: 'photo' }));
    // First switch: back → front
    act(() => {
      result.current.switchCamera();
    });
    expect(result.current.facing).toBe('front');

    // Second switch: front → back
    act(() => {
      result.current.switchCamera();
    });
    expect(result.current.facing).toBe('back');
...
describe('toggleRawMode', () => {
  it('toggles rawMode from false to true', async () => {
    ...
    await act(async () => {
      await result.current.toggleRawMode();
    });
...
describe('setSelectedMode', () => {
  it('sets selectedMode to video', () => {
    ...
    act(() => {
      result.current.setSelectedMode('video');
    });
...
describe('cycleTimerDuration', () => {
  it('cycles from 3 → 5 → 10 → 3', async () => {
    ...
    await act(async () => {
      await result.current.cycleTimerDuration();
    });
...
describe('setTimerDuration', () => {
  ...
  it('sets timerDuration to 5', () => {
    ...
    act(() => {
      result.current.setTimerDuration(5);
    });
...
describe('startRecording / stopRecording', () => {
  it('startRecording sets isRecording to true (requires cameraRef)', async () => {
    const mockRecordAsync = jest.fn().mockResolvedValue({});
    const cameraRef = { current: { recordAsync: mockRecordAsync } } as any;

    const { result: r } = renderHook(() => useCamera());
    // Replace cameraRef with our mock
    r.current.cameraRef.current = cameraRef.current;

    await act(async () => {
      await r.current.startRecording();
    });

    expect(r.current.isRecording).toBe(false); // finally block resets
  });
  ...
  it('stopRecording does nothing when not recording', async () => {
    ...
    // No cameraRef, so early return — no error
    expect(result.current.isRecording).toBe(false);
  });
});

describe('requestPermission', () => {
  it('returns permission object from useCameraPermissions', () => {
